...
const delayedIncDec = (n, mode, delay = 2500) => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			if (mode === "inc") {
				const nInc = n + 1;
				resolve(`${n} has been incremented to ${nInc}`);
			} else if (mode === "dec") {
				const nDec = n--;
				resolve(`${n} has been decremented to ${nDec}`);
			} else {
				reject(`"${mode}" is an invalid mode, please enter "inc" or "dec"`);
			}
		}, delay);
	});
};
...
delayedIncDec(3, "decrement", 2000)
	.then((value) => {
		console.log(value);
	})
	.catch((e) => {
